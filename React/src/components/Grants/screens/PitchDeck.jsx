...
const PitchDashboard = () => {
    ...
    useEffect(() => {
        const fetchPitches = async () => {
            ...
            try {
                ...
                if (
                    !grantsResponse?.data?.grants ||
                    !Array.isArray(grantsResponse.data.grants)
                ) {
                ...
                const pitchesPromises = grants.map(async (grant) => {
                    try {
                        const pitchesResponse = await axiosClient.get(
                            `grant/pitches/${grant.id}`
                        );
                        return pitchesResponse.data?.pitches || [];
                    } catch (error) {
                        console.error(
                            `Error fetching pitches for grant ${grant.id}:`,
                            error
                        );
                ...
                const cleanedPitches = combinedPitches.map((pitch) => ({
                    ...
                    totalGrantAmount: pitch.total_grant_amount
                        ...
                        : "N/A",
                    applicationDeadline:
                        pitch.application_deadline || "No deadline",
    ...
    const handleStatusChange = async (pitchId, newStatus) => {
        ...
        try {
            ...
            const endpoint = `grant/${action}/${pitchId}`;

            //  console.log("Making GET request to:", endpoint);
            const response = await axiosClient.get(endpoint);

            //  console.log("Backend Response:", response.data);
            toast.success(`grant ${newStatus.toLowerCase()} successfully`);
    ...
    const toggleFavorite = (id) => {
        setPitches(
            pitches.map((pitch) =>
                pitch.id === id
                    ? { ...pitch, favorite: !pitch.favorite }
                    : pitch
            )
        );
    };

    const messageBusinessOwner = (pitch) => {
        // Simulate sending a message
        console.log(
            `Message sent to ${pitch.startup_name} (${pitch.email})`
        );
    }
...
const filteredPitches = pitches.filter((pitch) => {
    ...
    if (filterDate !== "Any time") {
        const created = new Date(pitch.createdAt || pitch.created_at);
        const now = new Date();
        ...
        // Custom range: implement as needed
    }
                            ...
                            filteredPitches.length === 0 && (
                                <div className="flex flex-col items-center justify-center p-12">
                                    ...
                                    <p className="text-gray-500 text-center max-w-md">
                                        We couldn't find any pitches matching
                                            ...
                                            {filteredPitches.map((pitch) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-4 md:px-6 py-3 whitespace-nowrap">
                                                            <div
                                                                className={`flex items-center justify-center w-8 h-8 md:w-10 md:h-10 rounded-full ${getMatchColor(
                                                                    pitch.matchScore
                                                                )}`}
                                                            >
                                        ...
                                        <div className="mb-8">
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                                                    ...
                                                    <p className="text-sm font-medium text-gray-800">
                                                        {selectedPitch.totalGrantAmount ||
                                                            "N/A"}
                                                    </p>
                                                </div>
                                                <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                                                    ...
                                                    <p className="text-sm font-medium text-gray-800">
                                                        {selectedPitch.fundingPerBusiness ||
                                                            "N/A"}
                                                    </p>
                                                </div>
                                                <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                                                    ...
                                                    <p className="text-sm font-medium text-gray-800">
                                                        {selectedPitch.applicationDeadline ||
                                                            "No deadline"}
                                                    </p>
                                                </div>
                                            </div>
