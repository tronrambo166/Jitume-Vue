...
const ProjectManagers = () => {
  ...
  useEffect(()=> {
    const ProjectManagers = () => { 
        axiosClient.get('FindProjectManagers/'+bid_id)
          .then(({ data }) => {
  ...
  return (
    <div className="p-6 max-w-screen-xl mx-auto">
      ...
      {!token ? (
        <div className="mb-6">
         <div className="w-75 h-100 py-5 my-5 my-auto justify-content-center my-2 text-center mx-auto">
                <a style={{cursor:'pointer', width:'40%'}}
                    className="searchListing mx-auto text-center py-1 text-light font-weight-bold" data-target="#loginmodal2"
        ...
        {cards.length === 0 ? (
            <p className="text-center text-gray-500 col-span-full">No Manager available!</p>
          ) : (

            cards.map((card) => (
              <Link to={`/asset-service-details/${btoa(btoa(card.id))}/${bid_id}`} key={card.id} className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-shadow">
                <img src={'../'+card.image} alt={card.name} className="w-full h-38 object-cover" />
                <div className="p-4">
